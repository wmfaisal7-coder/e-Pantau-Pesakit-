...
export function DashboardPage({
  ...
  const patientStatusSummary = [
    {
      label: "Aktif",
      value: patients.filter((p) => p.patientStatus === "Aktif").length,
      tone: "success"
    },
    {
      label: "Tidak Aktif",
      value: patients.filter((p) => p.patientStatus === "Tidak Aktif").length,
      tone: "warning"
    },
    {
      label: "Selesai",
      value: patients.filter((p) => p.patientStatus === "Selesai Rawatan").length,
      tone: "info"
    }
  ];
  ...
  return (
    <div className="stack-lg">
      ...
      <div className={`alert-banner ${needFollowUp > 0 ? "danger" : ""}`}>
        ...
        <span>
          {needFollowUp > 0
            ? "Utamakan pesakit dengan status terlepas dan hari ini."
            : "Tiada tindakan segera diperlukan buat masa ini."}
        </span>
        {needFollowUp > 0 ? (
          <button className="secondary-btn table-btn" onClick={onOpenFollowUp}>
            Buka Follow-Up
          </button>
        ) : null}
      </div>
        ...
        <div className="card">
          ...
          <Table
            ...
            columns={[
              ...
              {
                header: "Pesakit",
                render: (row) => patients.find((p) => p.id === row.patientId)?.fullName ?? "-"
              },
        ...
        <div className="card">
          ...
          <Table
            ...
            columns={[
              ...
              {
                header: "Telefon",
                render: (row) => patients.find((p) => p.id === row.patientId)?.phoneNumber ?? "-"
              },
      ...
      <div className="card">
        ...
        <Table
          ...
          columns={[
            ...
            {
              header: "Status",
              render: (row) => <span className="pill danger">{getSystemStatus(row)}</span>
            }
          ]}
        />
      </div>
